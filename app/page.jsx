    ...
    <Layout>
      ...
      <section className="max-w-5xl mx-auto py-24 px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {[
            { ep: 45, time: "45 min", title: "Teaching Matters", desc: "Insights on building better classrooms and creating environments where every student can thrive." },
            { ep: 44, time: "32 min", title: "Career Clarity", desc: "Navigate your career path with advice grounded in experience and honest reflection." },
            { ep: 43, time: "50 min", title: "Work & Life Balance", desc: "Finding balance in a demanding world and making time for what truly matters." }
          ].map((episode) => (
            <div key={episode.ep} className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm hover:shadow-md transition-all group">
              ...
              <div className="mb-4">
                <span className="text-xs font-medium text-stone-500 uppercase tracking-wide">Episode {episode.ep} • {episode.time}</span>
              </div>
              <h3 className="text-xl font-serif text-stone-900 mb-3">{episode.title}</h3>
              <p className="text-stone-600 font-light text-sm mb-6 leading-relaxed">{episode.desc}</p>
              <a href="/podcast-episodes" className="inline-flex items-center gap-2 text-stone-900 font-medium text-sm hover:gap-3 transition-all">
                Listen Now
                <ArrowRight className="w-4 h-4" />
              </a>
